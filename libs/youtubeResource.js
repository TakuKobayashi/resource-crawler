...
exports.getInfoFromUrl = async function getInfoFromUrl(...urls) {
  const results = []
  const youtubeInfos = await Promise.all(urls.flatMap((url) => {
    return ytdl.getInfo(url)
  }));

  for (const youtubeInfo of youtubeInfos) {
    const maxSizeThumbnail = util.maxBy(youtubeInfo.player_response.videoDetails.thumbnail.thumbnails, (thumb) => thumb.width * thumb.height)
    results.push({
